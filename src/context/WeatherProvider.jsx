  ...
  // useEffect for automatic location and wetaher fetching
    ...
    useEffect(() => {
      ...
      let timeoutId = setTimeout(() => {
        // If it takes too long, fallback
        setErrorMessage("Location taking too long, showing London by default.");
        setGeoCode({ lat: 51.5072, lon: -0.1276 });
        setPlace("London");
        setLocation({ town: "London", country: "United Kingdom" });
        setSearching(false);
      ...
      const success = async (pos) => {
        ...
        try {
          ...
          // ✅ Use fallbacks in case city is undefined
          const city =
            placeData.city ||
            placeData.locality ||
          ...
          if (city) {
            setLocation({ town: city, country: placeData.countryName });
            setPlace(city);
            setErrorMessage(""); // clear any old errors
          } else {
            setErrorMessage("Couldn’t detect your location. Showing London.");
            setLocation({ town: "London", country: "United Kingdom" });
            setPlace("London");
          }
        } catch (err) {
          setErrorMessage("Location detection failed. Showing London by default.");
      ...
      const failure = (err) => {
        ...
        setSearching(false);
        if (err.code === 1) {
          setErrorMessage("Permission denied. Showing London by default.");
        } else {
          setErrorMessage("Couldn’t get your location. Showing London by default.");
        }
        setGeoCode({ lat: 51.5072, lon: -0.1276 });
        setPlace("London");
        setLocation({ town: "London", country: "United Kingdom" });
